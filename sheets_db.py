...
def sync_fred():
    ...
    if not records:
        ...
        try:
            ...
            print(f"    [sheets_db] FRED cold start failed ({e}) — returning empty")
    ...
    try:
        response = requests.get(url, timeout=15)
        response.raise_for_status()
        df_fresh = pd.read_csv(io.StringIO(response.text))
    except Exception as e:
        print(f"    [sheets_db] FRED gap-fill failed ({e}) — using cached data")
        return df
